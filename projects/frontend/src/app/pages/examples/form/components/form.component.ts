...
export class FormComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private store: Store, private translate: TranslateService, private notificationService: NotificationService) {}
  ...
  ngOnInit() {
    ...
    this.store.pipe(select(selectFormState), take(1)).subscribe(form => this.form.patchValue(form.form));
  }
  ...
  submit() {
    if (this.form.valid) {
      ...
      this.notificationService.info(
        (this.form.value.requestGift ? this.translate.instant("pldb.examples.form.text4") : this.translate.instant("pldb.examples.form.text5")) +
          " : " +
          this.translate.instant("pldb.examples.form.text6")
      );
    }
  }
