...
describe("TodosComponent", () => {
  ...
  const getTodoInput = () => fixture.debugElement.query(By.css("pldb-big-input input"));
  ...
  const getAddTodoButton = async () => {
    const buttons = await loader.getAllHarnesses(MatButtonHarness.with({ selector: "pldb-big-input-action button" }));
    return buttons[0];
  };
