...
export const selectTodosItems = createSelector(selectTodosState, state => state.items);

export const selectTodosFilter = createSelector(selectTodosState, state => state.filter);

export const selectTodos = createSelector(selectTodosItems, selectTodosFilter, (items, filter) => {
  if (filter === "ALL") {
    return items;
  } else {
...
export const selectRemoveDoneTodosDisabled = createSelector(selectTodosItems, items => !items.some(item => item.done));
